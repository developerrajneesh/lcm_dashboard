...
import { FiLink, FiSend, FiCheckCircle, FiArrowLeft } from "react-icons/fi";
import metaApi from "../../../utils/metaApi";

export default function LinkLaunch() {
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      const response = await metaApi.createLinkAd(adPayload);
    ...
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">Your link ad has been created and is ready to go live.</p>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-6 text-left max-w-md mx-auto">
            <div className="space-y-2 text-sm">
              ...
              <div className="text-pink-600 font-semibold"><strong>Ad ID:</strong> {adId || "N/A"}</div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={() => navigate("/user/meta-management/link/creative", { state: previousData })}
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 bg-pink-100 rounded-full flex items-center justify-center">
            <FiSend className="w-8 h-8 text-pink-600" />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Launch Your Link Ad</h1>
        ...
        <div className="flex justify-end">
          <button
            ...
            disabled={loading}
            className="px-6 py-2 bg-pink-500 text-white rounded-lg font-medium hover:bg-pink-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {loading ? (
              <>
